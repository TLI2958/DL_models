...
def train(model, optimizer, train_file_list):
    ...
    for epoch in pbar:
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = max(base_learning_rate *(decay_rate **(epoch//decay_step)), 
                                    learning_rate_clip)
        for j in range(num_train_file):
            total_loss = 0.0
            total_label_loss = 0.0
            total_seg_loss = 0.0
            total_label_acc = 0.0
            total_seg_acc = 0.0
            train_subset = load_data_(hdf5_data_dir, train_file_list[j])
            train_dataloader = torch.utils.data.DataLoader(train_subset, batch_size = args.batch, shuffle = True,
        ...
        if epoch % 2 == 0:
            ...
            eval(model, train_file_list, train_eval = True, epoch = epoch)
...
def eval(model, eval_ds, train_eval = False, epoch = 0):
    model.to(device)
    model.eval()
    ...
    with torch.no_grad():
        for j in range([num_test_file, num_train_file][train_eval]):
            eval_subset = load_data_(hdf5_data_dir, [test_file_list, train_file_list][train_eval][j])
            # if [test_file_list, train_file_list][train_eval] == train_file_list:
            eval_dataloader = torch.utils.data.DataLoader(eval_subset, batch_size = args.batch, shuffle = True,
                                                              collate_fn = partseg_train_collator(2048))
            # else:
                # eval_dataloader = torch.utils.data.DataLoader(eval_subset, batch_size = args.batch, shuffle = True, )
            for i, batch in tqdm(enumerate(eval_dataloader),
                ...
                loss, cls_loss, seg_loss = get_partseg_loss(l_pred = labels_pred, s_pred = seg_pred, 
                                                            ...
                                                            feat = end_points, reg_weight=0.001, weight = 1)
                # loss
                total_loss += loss.detach().to('cpu').item()
                total_label_loss += cls_loss.detach().to('cpu').item()
                total_seg_loss += seg_loss.detach().to('cpu').item()
        ...
        # total_seen = len(eval_dataloader) * [num_test_file, num_train_file][train_eval]
        avg_total_loss, avg_label_loss, avg_seg_loss = total_loss / total_seen, total_label_loss / total_seen, total_seg_loss / total_seen
        avg_label_acc, avg_seg_acc = total_label_acc / total_seen, total_seg_acc / total_seen
...
if __name__ == '__main__':
    ...
    if args.train:
        ...
        train(model, optimizer, train_file_list)
