...
def rotate_point_cloud(data,):
    ...
    rotation_matrix = np.array([[cosval, 0, sinval],
                                [0, 1, 0],
                                [-sinval, 0, cosval]])
    rotated_pc = np.dot(data.reshape((-1, 3)), rotation_matrix)
    return rotated_pc
...
def rotate_point_cloud_(batch_data, ):
    """ Rotate the point cloud along up direction with certain angle.
        ...
        Return:
          BxNx3 array, rotated batch of point clouds
    """
    B, N, C = len(batch_data), batch_data[0].shape[0], batch_data[0].shape[1]
    rotated_data = []
    for k in range(B):
        rotation_angle = np.random.uniform() * 2 * np.pi
        cosval = np.cos(rotation_angle)
        ...
        rotation_matrix = np.array([[cosval, 0, sinval],
                                    ...
                                    [-sinval, 0, cosval]])
        shape_pc = batch_data[k]
        rotated_data.append(torch.tensor(np.dot(shape_pc.reshape((-1, 3)), rotation_matrix),
                                      device = shape_pc.device))
    return rotated_data
...
def jitter_point_cloud(batch_data, sigma=0.01, clip=0.05):
    ...
    assert(clip > 0)
    for k in range(B):
        jittered_data.append(torch.tensor(batch_data[k] + torch.clamp(sigma * torch.randn(N, C), -1*clip, clip), 
                                        device = batch_data[k].device))
    return jittered_data


def augment_point_cloud(point_cloud, rotation_prob= 0.01, jitter_prob=0.01):
...
def augment_point_cloud_batch(point_cloud, rotation_prob= 0.01, jitter_prob=0.01):
    if np.random.uniform() < rotation_prob:
        # print('rotation applied...')
        point_cloud = rotate_point_cloud_(point_cloud)
    if np.random.uniform() < jitter_prob:
        # print('jittering applied...')
        point_cloud = jitter_point_cloud(point_cloud)
    return point_cloud   
...
class partseg_dataset_iter(torch.utils.data.IterableDataset):
    def __init__(self, data, label, seg):
        self.data = data
        self.label = label
        self.seg = seg
...
def load_data(hdf5_data_dir, train_file_list, dataset_type = 'iter'):
    ds = None
...
class partseg_train_collator:
    def __init__(self, max_length, rotation_prob = 0.05, jitter_prob = 0.05):
        self.max_length = max_length
        self.rotation_prob = rotation_prob
        self.jitter_prob = jitter_prob
        
    def __call__(self, batch,):
        batched_data = [torch.tensor(item['data']) for item in batch]
        batched_seg = [torch.tensor(item['segments']) for item in batch]
        batched_label = [torch.tensor(item['labels']) for item in batch]
        
        # aug_data = []
        # for k in range(len(batched_data)):
        #     aug = augment_point_cloud(batched_data[k], rotation_prob=rotation_prob, 
        #                               jitter_prob=jitter_prob)
        #     aug_data.append(torch.tensor(aug))
        aug_data = augment_point_cloud_batch(batched_data, rotation_prob = self.rotation_prob, 
                                             jitter_prob = self.jitter_prob)
        # Stack all data and segment tensors
        batched_data_tensor = torch.stack(aug_data, )
        batched_seg = torch.stack(batched_seg, )
        batched_label = torch.stack(batched_label, )
        
        return {'data': batched_data_tensor.to(torch.float32),  
                'labels': batched_label,
               'segments': batched_seg, }


def partseg_train_collator_(batch, rotation_prob = 0.01, jitter_prob = 0.01):
